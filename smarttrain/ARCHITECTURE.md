# SmartTrain — Smart Train Travel Reservation System

**SWE332 Software Architecture | Altınbaş University 2025–2026**
**Team Leader:** Soliman Sabawi (220513739)
...
### 7.2 Team Task Distribution

Each team member was assigned a specific area of responsibility. This division ensured parallel development with minimal conflicts and clear ownership.

| Team Member | Role | Responsibilities | Key Files |
|------------|------|-----------------|-----------|
| Soliman Sabawi | **Frontend Developer** | Designed and implemented the SPA user interface: home page, search form, train cards, seat map modal, booking modal, admin panel UI, and all CSS styling. Handled responsive design and micro-animations. | `index.html`, `style.css`, `animations.css`, `app.js` |
| MHD ALI AL TABAA | **Backend Developer** | Built the Express.js REST API: all route handlers, JWT authentication middleware, booking logic, admin endpoints, and server configuration. | `server.js`, `routes/auth.js`, `routes/trains.js`, `routes/bookings.js`, `routes/admin.js`, `middleware/auth.js` |
| ALBARAA BAROUDY | **Database Developer** | Designed and populated the in-memory data layer: train schedules, city connections, seat map generation, route coverage for all 28 city pairs, and data helper functions. | `data/db.js` |
| Mohamed Miloud Askri | **Debugger / Tester** | Tested all features end-to-end: search functionality (one-way and round-trip), booking flow, seat availability validation, admin panel, edge cases (empty results, invalid inputs), and cross-browser compatibility. Reported and verified bug fixes. | All files (testing scope) |

### 7.3 Main Package Diagram
